...
scripts/
  ...
  game/audio.gd         autoload: Audio.play("name"), music, volume buses
...
`Audio` is an autoload ([scripts/game/audio.gd](scripts/game/audio.gd)):

```gdscript
Audio.play("perfect")            # res://assets/audio/sfx/perfect.wav (or .ogg)
Audio.play("land", 0.9, 1.1)     # random pitch range so repeats vary
Audio.play_music("loop")         # res://assets/audio/music/loop.wav, looped
Audio.sfx_volume = 0.5           # buses: Master → Music, SFX
```

Sounds are looked up by file name, so adding one is: drop a `.wav`/`.ogg`
into `assets/audio/sfx/`, call `Audio.play("name")`. Missing files log a
warning instead of crashing.

The current sounds and the music loop are **placeholders generated by**
`tools/generate_placeholder_audio.py` (stdlib Python, chip-style synth).
Replace any of them by overwriting the file with the same name. Good free
sources: [jsfxr](https://sfxr.me/) for retro effects,
[freesound.org](https://freesound.org/) (check licences), or record your own.

Current hooks: `start`, `drop`, `land`, `perfect`, `miss`, `collapse`,
`splash`, `game_over`, and the `loop` music track (fades out on game over).
